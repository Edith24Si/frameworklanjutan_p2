...
export default function Booking() {
    ...
    const handleSave = (e) => {
        ...
        setData([{ id:`ORD-${String(data.length+1).padStart(3,"0")}`, ...form, totalBiaya:parseInt(form.totalBiaya)||0 }, ...data]);
        setShowForm(false);
            ...
            {showForm && (
                <div className="rounded-2xl p-6 border border-white/5" style={{ background:"#1a1d27" }}>
                    ...
                    <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[
                            { key:"pelanggan",  label:"Nama Pelanggan", placeholder:"Nama pelanggan", type:"text" },
                            { key:"layanan",    label:"Layanan",        placeholder:"Ganti Oli...",   type:"text" },
                            { key:"totalBiaya", label:"Total Biaya",    placeholder:"150000",         type:"number" },
                        ...
                        <div>
                            ...
                            <select className="w-full px-4 py-2.5 rounded-xl text-sm border border-white/5 outline-none text-gray-200"
                                ...
                                <option>Terjadwal</option><option>Dikerjakan</option><option>Selesai</option>
                            </select>
                        </div>
                        <button type="submit" className="md:col-span-2 py-2.5 rounded-xl font-bold text-white text-sm"
                            ...
                            {data.map((d, i) => (
                                <tr key={d.id} className="hover:bg-white/2 transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold" style={statusStyle(d.status)}>{d.status}</span>
                                    </td>
                                    <td className="px-4 py-3 text-sm font-bold text-white">Rp {d.totalBiaya.toLocaleString("id-ID")}</td>
